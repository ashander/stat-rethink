...
We seek to predict successful pirating attempts {\tt y} of all attempts {\tt n} based on the size {\tt P} and age {\tt A} of the pirate and the size {\tt V} of the victim.
...
I recenter the predictors PCTCOVER to {\tt pc} and FORESTAGE to {\tt fc}. 
...
#todo rewrite x axis to 0:100, see RM's code above
@ 
...
Averaged predictions really show no difference from the predictions using percent cover alone. 
This is because forestage is such a poor predictor. 

<<2b-avg, message=FALSE,cache=TRUE>>=
...
g <- ggplot(d)
g+geom_boxplot(aes(cover, SALAMAN))+geom_point(aes(cover, SALAMAN,size=FORESTAGE))
...
The variable SALAMAN binned across values of PCTCOVER.
Within each bin, SALAMAN displays overdispersion (variance exceeding the mean), while a Poisson distribution always has variance equal to the mean. 
This means that, even as a Poisson predictor, the single variate PCTCOVER is unable to explain the variance in salamander cuonts. 

If age of the tree cover (FORESTAGE) were a good predictor,  a regression based on both predictors {\em could} do a better job. 
Tree cover age, however, is a very poor predictor.
