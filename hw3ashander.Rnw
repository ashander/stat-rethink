...
height.PI = sapply(known.weights, function(x){HPDI(rnorm(n = 1e4, mean=post.samples$alpha + post.samples$beta*x, sd=post.samples$sigma), prob=0.9)})
...
heightY.CI = sapply(weight.range, function(x){HPDI(postY.samples$alpha + postY.samples$beta*x, prob=0.9)})
# calculate PI for mean
heightY.PI = sapply(weight.range, function(x){HPDI(rnorm(1e4, mean=(postY.samples$alpha + postY.samples$beta*x),sd=postY.samples$sigma), prob=0.90)})
...
d.f = Howell1
d.f$logw = log(d.f$weight)
mle.loghw <- mle2(height ~ dnorm(mean=alpha + beta*logw, sd=sigma), data=d.f, start=list(alpha=mean(d.f$height), beta=0, sigma=sd(d.f$height)))
precis(mle.loghw)
@ 
...
heightf.CI = sapply(weight.range, function(x){HPDI(post.full.samples$alpha + post.full.samples$beta*log(x), prob=0.95)})
# calculate PI for mean
heightf.PI = sapply(weight.range, function(x){HPDI(rnorm(1e4, mean=(post.full.samples$alpha + post.full.samples$beta*log(x)),sd=post.full.samples$sigma), prob=0.95)})
