...
set.seed(100) # for reproduciblity of results based on sampling
@ 
...
From this model, I estimate $\alpha$ and $\alpha_L$ using a GLM: 
...
The above figure plots densities for estimated probabilty of mortality. 
...
mean(p.death.diff) # mean change in probabilty of death with large stock
HPDI(p.death.diff, 0.95) # interval for change in probabilty of death with large stock
@ 
...
The BIC weight corresponds to the approximate posteior probabilty of the model, conditional on only these two models being considered. 
...
The figure above plots the empirical proportion of mortaility across all observations of small (grey triangles) and large (black circles) stocks.
...
The figure above plots the empirical proportion of mortaility across all observations of small (triangles) and large (circles) stocks, with MLE mean (solid) and 95\% confidence interval (dashed) for proportion of surviving.
Dot size is proportional to the number of observations. 
...
Now, I model counts of {\tt mortality} across the whole data set, as predicted by stock size, and with varying intercepts clustered by herd.
Specifically, I model
\begin{align*}
m_i  &\sim {\rm Binomial} (p_i, n_i),\\
...
With this model, I estimate $\alpha$, $\alpha_L$ and $\sigma_\alpha$, the variance in the varying effects on {\tt herd}. 
...
Thus, the inference from the mixed effects model is opposite to that from the first model, which didn't allow for cluster-level differences.

There is, however, substantial variation among herds.
The estimated standard deviation of the random effects among the herds is 0.56.
...
g <- g +geom_point(aes(herd, mortality/n,color=stock), size=3,data=d) + scale_color_grey() + opts(legend.position='none')
...
#ggplot(d)+geom_boxplot(aes(stock, mortality/n))+theme_bw()
...
grid.arrange(g8, g, g7 + geom_point(aes(n.large/(n.small+n.large), logistic(a.j)), size=3,alpha=0.6) + xlim(c(0.5,1)) + ylim(c(0,1)) + xlab('proportion large stock'), ncol=2)
