...
pd.under.null <- dbinom(die.succ:die.trials, size=die.trials, prob=pd.h0)
pd.pval <- sum(pd.under.null)
pd.pval
@ 
...
<<bayes-die-loaded, message=FALSE>>=
pr.loaded_data <-pr.data_loaded*pr.loaded/pr.data
pr.loaded_data
pr.fair_data.norm <- pr.fair_data/(pr.fair_data + pr.loaded_data)
pr.fair_data.norm
@ 
