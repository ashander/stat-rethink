...
d2$upper <- ifelse(d2$district > 30, FALSE, TRUE)
...
g + geom_text(aes(as.factor(district),c(1.025),label=population), size=3, fontfamily='serif')
...
p.corr$lab <- as.character(unique(d2$district))

plot(1,1, ylim=c(0,1), xlim=c(0,1), pch='', xlab='probability contraceptive use rural', ylab='probability contraceptive use urban')
abline(0,1, lwd=2,col='grey', lty=2)
points(rural,urban, col=col.alpha('grey',0.6),pch=19)
#find outliers
labs <- p.corr[c(1,4,11,27,34,41,52),]
text(labs$rural, labs$urban, labels=labs$lab)
@ 
...
g3+geom_point(aes(age,height,size=Subject))
@ 
